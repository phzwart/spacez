...
class GaussianBlur3D(nn.Module):
    ...
    with inputs having multiple channels and batch sizes, applying the same
    sigma (standard deviation) across all channels.
    ...
    Args:
        initial_sigma (float): The initial value of the standard deviation for
        the Gaussian kernel.
        max_kernel_size (int, optional): The maximum size of the Gaussian
        kernel. Defaults to 11.
        grads (bool, optional): Whether to set gradients for the sigma on or
    ...
    def create_gaussian_kernel(self, kernel_size, sigma, device="cpu"):
        ...
        Args:
            ...
            device (str, optional): The device to create the kernel on.
            Defaults to 'cpu'.
    ...
    def forward(self, x):
        ...
        self.kernel = self.create_gaussian_kernel(
            self.max_kernel_size, sigma, device
        ).to(device)
...
class GaussianBlur2D(nn.Module):
    ...
    This class provides a mechanism to apply Gaussian blurring to 2D tensors
    (e.g. images), supporting multiple channels (e.g., RGB images) and various
    batch sizes. The same standard deviation (sigma) is applied across all
    channels to maintain consistency in blurring effect.

    Attributes:
        sigma (nn.Parameter): The standard deviation of the Gaussian kernel.
        max_kernel_size (int): The maximum size of the kernel used for
        blurring. kernel (Tensor): The computed Gaussian kernel tensor.

    Args:
        initial_sigma (float): The initial standard deviation value for the
        Gaussian kernel.
        max_kernel_size (int, optional): The maximum dimension of the Gaussian
        kernel. Defaults to 11.
        grads (bool, optional): Whether to set gradients for the sigma on or
        not. False by default
    """
    ...
    def create_gaussian_kernel(self, kernel_size, sigma, device="cpu"):
        """
        Generates a normalized 2D Gaussian kernel using the specified sigma and
        kernel size.

        Args:
            kernel_size (int): The size of the kernel.
            sigma (Tensor): The sigma (standard deviation) value for the
            Gaussian distribution.
            device (str, optional): The device (CPU or GPU) where the kernel
    ...
    def forward(self, x):
        ...
        This method pads the input tensor to handle borders properly, updates
        the Gaussian kernel if necessary,
        ...
        Returns:
            Tensor: The blurred tensor, having the same dimensions as the
            input.
        """
        sigma = self.sigma.abs() + 1e-6
        device = x.device
